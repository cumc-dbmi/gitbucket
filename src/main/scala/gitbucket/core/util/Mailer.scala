...
class Mailer(settings: SystemSettings) {
  ...
  def subjectFor(subject: String, to: String): String = {
    ...
    if (LDAPUtil.Whitelist.contains(domain)) subject else s"$subject ${LDAPUtil.EncryptFlag}"
  }
  ...
  def send(
    ...
    createMail(subjectFor(subject, to), textMsg, htmlMsg, loginAccount).foreach { email =>
      email.addTo(to).send
    }
  }
